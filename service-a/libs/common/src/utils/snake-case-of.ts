...
export function snakeCaseOf<T extends Object>(
  ...
  if (fnStr.startsWith('class ') && !fnStr.startsWith('class =>')) {
    return cleanseAssertionOperators(
      toSnakeCase(fnStr.substring('class '.length, fnStr.indexOf(' {')))
    );
  }
  if (fnStr.includes('=>')) {
    return cleanseAssertionOperators(
      toSnakeCase(fnStr.substring(fnStr.indexOf('.') + 1))
    );
  }
  const matchRegex =
    /function\s*\(\w+\)\s*\{[\r\n\s]*return\s+\w+\.((\w+\.)*(\w+))/i;
  ...
  if (fnStr.startsWith('function ')) {
    return cleanseAssertionOperators(
      toSnakeCase(fnStr.substring('function '.length, fnStr.indexOf('(')))
    );
  }
